...
tcLcStmt m_tc ctxt (TransStmt { trS_form = form, trS_stmts = stmts
  ...
  = do { let (bndr_names, n_bndr_names) = unzip bindersMap
       ...
       ; let n_app = case form of
             ...
             by_arrow = case by' of
                          Nothing       -> \ty -> ty
                          Just (_,e_ty) -> \ty -> (alphaTy `mkVisFunTyU` e_ty) `mkVisFunTyU` ty
             ...
             using_poly_ty = mkInvForAllTy alphaTyVar $
                             by_arrow $
                             poly_arg_ty `mkVisFunTyU` poly_res_ty
...
tcMcStmt ctxt (TransStmt { trS_stmts = stmts, trS_bndrs = bindersMap
  ...
  = do { let star_star_kind = liftedTypeKind `mkVisFunTyM` liftedTypeKind
       ...
       ; let by_arrow :: Type -> Type
             ...
             by_arrow = case by of
                          Nothing -> \res -> res
                          Just {} -> \res -> (alphaTy `mkVisFunTyU` by_e_ty) `mkVisFunTyU` res
       ...
       ; (_, bind_op')  <- tcSyntaxOp MCompOrigin bind_op
                             ...
                             , synKnownType (n_app tup_ty `mkVisFunTyU` new_res_ty) ]
                             res_ty $ \ _ -> return ()
       ...
       ; fmap_op' <- case form of
                       ...
                       _ -> fmap unLoc . tcPolyExpr (noLoc fmap_op) $
                            ...
                            (alphaTy `mkVisFunTyU` betaTy)
                            `mkVisFunTyU` (n_app alphaTy)
                            `mkVisFunTyU` (n_app betaTy)
        ...
        ; ((_, mfix_op'), mfix_res_ty)
            <- tcInferInst $ \ exp_ty ->
               tcSyntaxOp DoOrigin mfix_op
                          [synKnownType (mkVisFunTyU tup_ty stmts_ty)] exp_ty $
               \ _ -> return ()
...
tcApplicativeStmts ctxt pairs rhs_ty thing_inside
 = do { body_ty <- newFlexiTyVarTy liftedTypeKind
      ...
      ; let fun_ty = mkVisFunTysU pat_tys body_ty
