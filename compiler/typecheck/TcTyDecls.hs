...
synonymTyConsOfType ty
  ...
  where
     ...
     go (FunTy _ m a b)   = go m `plusNameEnv` go a `plusNameEnv` go b
...
irType = go
  where
    ...
    go lcls (ForAllTy tvb ty)  = do { let tv = binderVar tvb
                                    ...
                                    ; go lcls' ty }
    go lcls (FunTy _ m arg res)= markNominal lcls m >> go lcls arg >> go lcls res
    go _    (LitTy {})         = return ()
...
markNominal lcls ty = let nvars = fvVarList (FV.delFVs lcls $ get_ty_vars ty) in
  ...
  where
    ...
    get_ty_vars (FunTy _ m t1 t2)   = get_ty_vars m `unionFV` get_ty_vars t1 `unionFV` get_ty_vars t2
